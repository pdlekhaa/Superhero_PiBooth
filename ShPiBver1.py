...
def show_image(file_path):  # Added this function
	try:
		...
		geometry = "640x960+1200+100" 
		subprocess.run(['feh', '--no-fehbg', '-x','--geometry', geometry, file_path])  # Using subprocess to open feh
...
def add_frame(background, choice): #takes in openCV image (of picture taken)
	...
	if 'Expression' in choice:
		foreground = cv2.imread("/home/pi1/Pink_Comic_CFE_Final_2.png", cv2.IMREAD_UNCHANGED) 
		...
		foreground = cv2.imread("/home/pi1/Green_Comic_CSJ_Final_2.png", cv2.IMREAD_UNCHANGED) 
...
def send_email(processed_img_path):
	...
	<html>
	  <body>
	    <p> Hello Superhero,
	    <p>Thanks for visiting the PCAS SuperHero PiBooth Experience!<br>
	    PCAS offers two minors: <br>
	    <li><a href="https://docs.google.com/document/d/1bdLxFaVjTvOoGUqpJmqCrrxjacouaLTe1tCdrDaotSs/preview?pli=1&tab=t.0">Computing for Scientific Discovery </a></li>
	    <li><a href="https://docs.google.com/document/d/1r8NmDGjydaTOzAgB1licdX0NEDQptMC89utYjJqg8pU/preview?tab=t.0">Computing for Expression </a></li>
	    You can schedule an advising appointment <a href="https://myadvising.lsa.umich.edu/appointments/offices/PCAS">here.</a><br>
	    Or, if prepared to declare, you can start the process <a href="https://lsa.umich.edu/computingfor/undergraduates/advising/declare-a-pcas-minor.html">here.</a> <br> 
	    <br>
	    Best wishes from PCAS!!
	    </p>
	  </body>
	</html>
	"""
...
def thanks_and_next_steps(win):  # Define stdscr as the parameter
	# Use stdscr to get the full dimensions of the screen
	time_delay =.01
	...
	prompt1 = "Thanks for visiting the PCAS SuperHero PiBooth!!"
	spool_text(win, prompt1, 2, 3, time_delay)
	prompt2 = "Check your email for advising information."
	spool_text(win, prompt2, 2, 5, time_delay)
	prompt3 = "Wishing you a Super day!"
	spool_text(win, prompt3, 2, 7, time_delay)
	time.sleep(1)
	prompt4 = "NEXT !!!" 
	win.addstr(10, win_width//2 - len(prompt4)//2 , prompt4, curses.color_pair(3) | curses.A_BLINK)
	win.refresh()
	time.sleep(5)
...
def on_button_pressed(win, student_info):
	...
	prompt = "Approve the photo or retake?"  # New prompt text
	spool_text(win, prompt, 2, 2, 0.1)  # New text display call
	user_choice = handle_approve_response2(win)  # New call to handle user response

	if user_choice == "Approve":  # New logic branch
		print_image(processed_img_path)
		send_email(processed_img_path)
		thanks_and_next_steps(win)
	elif user_choice == "Retake":  # New logic branch
		win.clear()
		win.refresh()
		on_button_pressed(win,student_info)
...
def draw_approve_menu(win, current_idx, options):
	""" Draws a horizontal Approve/Retake menu with highlight """
	height, width = win.getmaxyx()
	menu_y = height - 3  # Position menu a few lines from bottom
	for idx, option in enumerate(options):
		x = width // 2 - len(" ".join(options)) // 2 + (len(option) + 4) * idx  # center horizontally
		if idx == current_idx:
			win.attron(curses.color_pair(3))  # Highlight current selection
			win.addstr(menu_y, x, option)
...
def handle_approve_response2(win):
	""" Function to handle approve/retake response """
	choices = ["Approve", "Retake"]
	current_choice_idx = 0
	draw_approve_menu(win, current_choice_idx, choices)
	...
	while True:
		...
		if key in (ord('a'), ord('A')): 
			current_choice_idx = 0  # Go to Approve
		elif key in (ord('r'), ord('R')):
			current_choice_idx = 1  # Go to Retake
		elif key == (94 and 91 and 91 and 68) and current_choice_idx > 0: 
			current_choice_idx -= 1
		elif key == (94 and 91 and 91 and 67) and current_choice_idx < len(choices) - 1:
			current_choice_idx += 1
			...
			if y < height-1:
				...
				if 0 <= y < height and 0 <= x < width:
					...
					if any(y >= height-1 for y in column_positions):
						...
						win.box()
						win.attroff(curses.color_pair(4)) 
						win.addstr(win_height//2,win_width//2-len("press any key")//2,"press any key")
						win.refresh()
						stdscr.refresh()
...
def main(stdscr):
	...
	prompt5 = f"and email your photo!!"
	spool_text(win,prompt5,2,5,time_delay)  
	
	prompt5a = f"The routers in the Union are very busy!!"
	spool_text(win,prompt5a,2,7,time_delay) 
	
	prompt5b = f"It may take up to 3 minutes to arrive in your inbox."
	spool_text(win,prompt5b,2,8,time_delay) 
	
	prompt5b = f"Press any key."
	spool_text(win,prompt5b,3,11,time_delay)  
	...
	back_arrow = u'\u2190'
	forward_arrow = u'\u2192'
	# reply0 = handle_yes_no_response(win)
	# win.attron(curses.color_pair(4))
	# win.border()
	# win.attroff(curses.color_pair(4))
	# win.refresh()
	...
	while True:
		...
		if key == (94 and 91 and 91 and 65):
			...
			draw_menu(win, current_row_idx, menu_items)
			win.addstr(2,1,"                                        ")
			win.addstr(4,2,f"up arrow pressed {up_arrow}",curses.color_pair(2))
			win.refresh()
			...
			draw_menu(win, current_row_idx, menu_items)
			win.addstr(2,1,"                                        ")
			win.addstr(4,2,f"down arrow pressed {down_arrow}",curses.color_pair(2))
			win.refresh()
			...
			draw_menu(win, current_row_idx, menu_items)
			win.addstr(2,2,"                                       ")
			win.addstr(4,2,f"back arrow pressed {back_arrow}",curses.color_pair(2))
		elif key == (94 and 91 and 91 and 67): 
			forward_arrow = u'\u2192'
			current_row_idx = (current_row_idx + 1) % len(menu_items)
			draw_menu(win, current_row_idx, menu_items)
			win.addstr(2,2,"                                       ")
			win.addstr(4,2,f"forward arrow pressed {forward_arrow}",curses.color_pair(2))	
		elif key == curses.KEY_ENTER or key in [10, 13]:
			# Enter key to select
